...
public final class GameplayEvents {
    ...
    public static void tapResinTree(PlayerInteractEvent.RightClickBlock event) {
        if (event.getFace() == null || !event.getLevel().getBlockState(event.getPos()).is(ModTags.RESIN_TREES)) return;
        ItemStack tool = event.getEntity().getItemInHand(event.getHand());
        if (!tool.is(ModTags.PRIMITIVE_KNIVES) && !tool.is(ModTags.RESIN_TAPPING_TOOLS)) return;
        var scarPos = event.getPos().relative(event.getFace());
        if (!event.getLevel().getBlockState(scarPos).canBeReplaced()) return;
        event.setCancellationResult(InteractionResult.sidedSuccess(event.getLevel().isClientSide));
        event.setCanceled(true);
        if (event.getLevel().isClientSide) return;
        event.getLevel().setBlock(scarPos, ModBlocks.RESIN_SCAR.get().defaultBlockState()
                .setValue(ResinScarBlock.FACING, event.getFace().getOpposite()), Block.UPDATE_CLIENTS);
        event.getLevel().playSound(null, scarPos, SoundEvents.AXE_STRIP, SoundSource.BLOCKS, .55F, 1.25F);
        if (!event.getEntity().hasInfiniteMaterials()) {
            EquipmentSlot slot = event.getHand() == net.minecraft.world.InteractionHand.MAIN_HAND ? EquipmentSlot.MAINHAND : EquipmentSlot.OFFHAND;
    ...
    public static void igniteCharcoalMound(PlayerInteractEvent.RightClickBlock event) {
        ...
        event.setCancellationResult(InteractionResult.sidedSuccess(event.getLevel().isClientSide)); event.setCanceled(true);
        if (!probe.isValid()) { if (probe.failureReason() != null) event.getEntity().displayClientMessage(probe.failureReason(), true); return; }
